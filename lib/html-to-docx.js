...
(function () {
  ...
  function convertBlockElement(element, ctx, listLevel) {
    ...
    // 跳过知乎目录导航区域
    if (hu.isCatalog(element)) return blocks;

    // 跳过知乎参考文献列表（已通过尾注方式嵌入）
    if (hu.isReferenceList(element)) return blocks;

    // 标题
    const hMatch = tag.match(/^h([1-6])$/);
    if (hMatch) {
      // 跳过"参考"标题（当后面紧跟 ReferenceList 时，已通过尾注嵌入）
      if (hu.isReferenceList(element.nextElementSibling)) return blocks;
      const headingMap = {
    ...
    // 块级公式
    if (hu.isMath(element)) {
      const latex = hu.getLatex(element);
      if (latex) {
        const mathRuns = convertLatexToDocxMath(latex);
        if (mathRuns) {
    ...
    // 知乎视频
    if (hu.isVideo(element)) {
      const info = hu.getVideoInfo(element);
      blocks.push(new Paragraph({
        children: [
    ...
    // 链接卡片
    if (hu.isLinkCard(element)) {
      const info = hu.getLinkCardInfo(element);
      blocks.push(new Paragraph({
  ...
  async function htmlToDocx(htmlString, options) {
    ...
    for (const en of ctx.endnotes) {
      const enChildren = [new TextRun({ text: en.text, size: 20 })];
      if (en.url) {
