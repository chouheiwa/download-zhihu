...
(function () {
  ...
  function collectInlineElements(node, style, ctx) {
    ...
    if (tag === 'sup' && node.getAttribute('data-text')) {
      ...
      const noteUrl = node.getAttribute('data-url') || '';
      const endnoteId = ctx.endnotes.length + 1;
      ctx.endnotes.push({ id: endnoteId, text: noteText, url: noteUrl });
  ...
  function convertBlockElement(element, ctx, listLevel) {
    ...
    // 跳过知乎目录导航区域
    if (element.classList?.contains('Catalog') ||
        element.classList?.contains('Catalog-content') ||
        element.querySelector?.(':scope > .Catalog-content')) {
      return blocks;
    }

    // 跳过知乎参考文献列表（已通过脚注方式嵌入）
    if (element.classList?.contains('ReferenceList')) {
      return blocks;
    }
    ...
    if (hMatch) {
      // 跳过"参考"标题（当后面紧跟 ReferenceList 时，已通过脚注嵌入）
      const nextSibling = element.nextElementSibling;
      if (nextSibling?.classList?.contains('ReferenceList')) {
        return blocks;
      }
      const headingMap = {
